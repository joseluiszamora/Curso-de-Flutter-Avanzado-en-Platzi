...
class ProfileHeader extends StatelessWidget {
  ...
  Widget showProfileData (AsyncSnapshot snapshot) {
    if(!snapshot.hasData || snapshot.hasError) {
      ...
      return Container(
        ...
        child: Column(
          children: <Widget>[
            ...
            Text("No se pudo cargar la info., make login")
          ],
        )
      );
    } else {
      print("Logueado");
      print(snapshot.data);
      user = new User(uid: null, name: snapshot.data.displayName, email: snapshot.data.email, photoURL: snapshot.data.photoUrl);
      final title = Text(
        'Profile',
        style: TextStyle(
          fontFamily: 'Lato',
